...
class CardDatabase:
    ...
    def find_best_match(self, ocr_name, collector_ocr=""):
        if not ocr_name:
        	return None
        ...
        if name_low in self.db:
            ...
            # Fuzzy match (supports OCR typos for both languages)
            match = get_close_matches(name_low, self.valid_names, n=1, cutoff=0.75)
            if not match:
            	return None
